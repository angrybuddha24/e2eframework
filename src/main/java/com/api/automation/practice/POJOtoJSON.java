...
public class POJOtoJSON {
    public void convertPOJOtoJSONUsingGSON() {
        ...
        /* When values are passed through both constructor and set menthod, the set method values takes preference and 
        the output is {"name":"abc","age":26,"isStudent":true} 
        
        When values are passed only via constructor, the constructor values are considered and 
        the output is {"name":"def","age":22,"isStudent":false} 
        
        When some null values are passed via constructor, gson excludes the key while forming the json and 
    ...
    /* When some null values are passed via constructor, jackson includes the key while forming the json and sets the value as null and 
